...
library(MASS)
library(glmnet)
>>>>>>> 1cb88371370677146e084f9cd8e5b3db982a5b87
...
best_summary_df = 
  ...
  remove_rownames() %>% 
  add_column(" " = c(1:9), .before = "status_Developed")
...
```{r}
mod_3 = lm(le ~ adult_mort + bmi + under_five + polio_high + hiv_aids + gdp + HDI + schooling, data = le_df)
```
...
mod_4 = glmnet(x, y, alpha = 1, lambda = best_lasso_lambda)
coef(mod_4)
...
cv_df = 
  ...
  mutate(
    ...
    mod_4 = map(train, ~glmnet(x, y, alpha = 1, lambda = best_lasso_lambda, data = .x)),
    mod_5 = map(train, ~glmnet(x, y, alpha = 0,data = .x))
    ) %>% 
  mutate(
    ...
    rmse_4 = map2_dbl(mod_4, test, ~modelr::rmse(model = full_model, data = .y)),
    rmse_5 = map2_dbl(mod_5, test, ~modelr::rmse(model = full_model, data = .y))
    ) %>% 
  mutate(
    ...
    mae_4 = map2_dbl(mod_4, test, ~modelr::mae(model = full_model, data = .y)),
    mae_5 = map2_dbl(mod_5, test, ~modelr::mae(model = full_model, data = .y))
    ) %>% 
  mutate(
    ...
    rsq_4 = map2_dbl(mod_4, test, ~modelr::rsquare(model = full_model, data = .y)),
    rsq_5 = map2_dbl(mod_5, test, ~modelr::rsquare(model = full_model, data = .y))
    )
```
