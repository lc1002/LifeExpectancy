...
mod_4 = lm(le ~ adult_mort + bmi + under_five + polio_high + hiv_aids + gdp + HDI + schooling, data = le_df)
...
cv_df = 
  ...
  mutate(
    mae_1 = map2_dbl(mod_1, test, ~mae(model = .x, data = .y)),
    mae_2 = map2_dbl(mod_2, test, ~mae(model = .x, data = .y)),
    mae_3 = map2_dbl(mod_3, test, ~mae(model = .x, data = .y)),
    mae_4 = map2_dbl(mod_4, test, ~mae(model = .x, data = .y))
    ) %>% 
  mutate(
    rsq_1 = map2_dbl(mod_1, test, ~rsquare(model = .x, data = .y)),
    rsq_2 = map2_dbl(mod_2, test, ~rsquare(model = .x, data = .y)),
    rsq_3 = map2_dbl(mod_3, test, ~rsquare(model = .x, data = .y)),
    rsq_4 = map2_dbl(mod_4, test, ~rsquare(model = .x, data = .y))
    )
```
