...
From the correlation matrix, we can see that life expectancy has a high positive correlation with `HDI` and `schooling`, and a high negative correlation with `adult_mort` and `hiv_aids`. Indeed,  we would expect the outcome variable, life expectancy, to be highly impacted by these predictors.

We can also see that many of our variables are moderately correlated with each other. For example, `HDI` is moderately correlated with multiple variables. It is positively related to `alcohol`, `percent_exp`, `bmi`, `gdp`, `status_Developing`, `polio_low`,  `hepatitis_b_low` and `diphtheria_low` , and negatively related to `adult_mort`, `status_Developed`, `thin_1_19`, `thin_5_9`,`polio_high`,  `hepatitis_b_high` and `diphtheria_high`. Some of our variables are also highly correlated with each other. For example, `gdp` has a strong positive correlation with `percent_exp`. In addition, dummy variables created from the same categorical variable are also highly correlated (e.g. `status_Developed` and `status_Developing`). Therefore, multicollinearity must be taken into consideration when selecting our model.
...
Automated selection procedures are often used when there are many potential variables available in the data, and there are uncertainties in selecting the most appropriate variables for a reliable prediction. Using automatic model selection performed in R, we have generated four models. Outlined below are the four automatic model selection procedures performed: **stepwise selection**,**backward/forward selection**, **best subsets selection**, **lasso regression** and **ridge regression**.
...
Since backward, forward and stepwise selection generated the same model, we will only include one of them for further analysis. For simplicity, we will label this model as stepwise model.
...
Here is our final best subset model:
...
First, we will use `crossv_mc()` to perform the training/testing split and generate 100 random training and testing datasets. Then, we will fit all candidate models to the training data and obtain the corresponding RMSE, R^2 and MAE for the testing data using `map()` and `map2_dbl()`. Finally, we will plot the distribution of RMSE, R^2 and MAE of all models using violin plots, and use them to visualize and compare the performance of the candidate models.
