...
```{r include=FALSE}
#stepwise
mod_1 = lm(le ~ status + adult_mort + infantdeaths + bmi + polio + diphtheria + hiv_aids + gdp + thin_1_19 + HDI + schooling, data = le_df)

#health
mod_2 = lm(le ~ polio + total_exp + diphtheria + hiv_aids + thin_1_19, data = le_df)

#full model
mod_3 = lm(le ~ status + adult_mort + bmi + polio + diphtheria + hiv_aids + gdp + thin_1_19 + HDI + schooling, data = le_df)

# health-related factors, mortality factors, economical factors and social factors
mod_4 = lm(le ~ status + percent_exp + HDI + adult_mort + infantdeaths + schooling, data = le_df)

#backward
mod_5 = lm(le ~ status + adult_mort + bmi+ polio + diphtheria + hiv_aids + gdp + thin_1_19 + HDI + schooling, data = le_df)

#forward
mod_6 = lm(le ~ adult_mort + infantdeaths + alcohol + bmi + polio + diphtheria + hiv_aids + gdp + thin_5_9 + HDI + schooling, data = le_df)

#best subsets selection with 8 variables
mod_7 = lm(le ~ adult_mort + infantdeaths + polio + hiv_aids + gdp + HDI + schooling, data = le_df)

## Best Subsets model with 7 variables
mod_8 = lm(le ~ adult_mort + bmi + polio + hiv_aids + gdp + HDI + schooling, data = le_df)
```
